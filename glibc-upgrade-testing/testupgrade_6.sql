...
create index test_id1 on test_character_type(char_1);
create index test_id2 on test_character_type(varchar_10);
create index test_id3 on test_character_type(txt);
...
create table test1(content varchar) DISTRIBUTED by (content);
insert into test1 (content) values ('a'), ('$a'), ('a$'), ('b'), ('$b'), ('b$'), ('A'), ('B');
create index id1 on test1(content);

----  case4 test speical case with '""'
CREATE TABLE hash_test (id int, date text) DISTRIBUTED BY (date);
...
insert into test2 values (2, '02-1'), (2, '03-1'), (2, '08-1'), (2, '09-01'), (1, '11'), (1, '1-1');
...
-- case6 for testing insert into root select * from partition_range_test where date > '"02"';
create table root (id int, date text) DISTRIBUTED BY (id)
PARTITION BY RANGE (date)
(PARTITION Jan START ( '01') INCLUSIVE ,
PARTITION Feb START ( '02') INCLUSIVE ,
PARTITION Mar START ( '03') INCLUSIVE
...
insert into root select * from partition_range_test where date > '"02"';

--- case7 test range partition with special character '“”' with ao
...
--- case8 for index constraint violation 
...
-- case9 for index unique constraint violation 
