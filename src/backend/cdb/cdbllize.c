...
typedef struct PlanProfile
{
	...
	Flow	   *currentPlanFlow;	/* what is the flow of the current plan
									 * node */
	bool	visited;            /* indicate first traverse the node*/
} PlanProfile;
...
prescan_walker(Node *node, PlanProfile *context)
{
	...
	if (is_plan_node(node))
	{
		...
		*/
        ...
        else if (plan->flow && plan->flow->req_move != MOVEMENT_NONE)
        {
            /*
        ...
        {
            /* if it is not the top-level node for slices, ignore setting the
             * context->currentPlanFlow
             */
        }
